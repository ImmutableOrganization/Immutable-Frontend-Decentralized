...
const { chains, provider } = configureChains(
  [chain.arbitrum],
  [
    alchemyProvider({ apiKey: 'E4mZVO8Fc9ZjEIisFEZzQfRW-3iI3K8M', priority: 1 }),
...
function MyApp({ Component, pageProps }: AppProps) {
  ...
  const { getSocketId, setUserWantsSocketOn, disconnectSocket, userCounter, channelsRef, socketConnected, selectChannel, setEnableChannelWhiteList, enableChannelWhiteList, channelInfo, sendChatMessage, joinRoom, leaveRoom, selectedChannel } = useSocket();
      ...
      <PopupContext.Provider value={{ formError, loading, setFormError, setIsLoading, openToast, toastMessage, setToastMessage, setOpenToast, toastType, setToastType }}>
        <WagmiConfig client={client} >
          ...
          <div key={router.asPath} className={'top-margin ' + ((formError.open || loading) ? 'blurred' : '')} >
            <Component {...pageProps}
              // for chat
              socketId={getSocketId()} setUserWantsSocketOn={setUserWantsSocketOn} disconnectSocket={disconnectSocket} userCounter={userCounter} channelsRef={channelsRef} socketConnected={socketConnected}
              selectChannel={selectChannel} setEnableChannelWhiteList={setEnableChannelWhiteList} enableChannelWhiteList={enableChannelWhiteList} channelInfo={channelInfo}
              sendChatMessage={sendChatMessage} joinRoom={joinRoom} leaveRoom={leaveRoom} selectedChannel={selectedChannel}
            />
          </div>
        </WagmiConfig>
