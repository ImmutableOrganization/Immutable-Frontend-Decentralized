...
const immutableCollectionDetails = () => {
    return (
        <>
            <ul>Each NFT will vary based on the category fields below. % represents the percent chance of this property being selected in that category.
                ...
                <li>Divisibility factors: [1, 2, 3, 5, 7, 11, 13, 17]. 30% chance for first 3 numbers, 30% chance for middle 3 numbers, 30% chance for last 3 numbers, 10% probability for all numbers. </li>
...
export const GeneratedImageModal: React.FC<GeneratedImageModalInterface> = ({ imageSrc, setCloseGeneratedImage, metadata }) => {
    const generatedImageBody = () => {
        return (
            <>
                <ShowCaseItem name="0x5" hue={metadata.hue} stepFactor={[metadata.stepAmount]} divisibilityFactor={metadata.divisibilityFactor} image={imageSrc} />
                <input type='button' className="button" value='CLOSE' onClick={() => setCloseGeneratedImage(true)} />
            </>
        )
    }
    return (
        <Frame className="generatedImage" headerText="GENERATED IMAGE" body={generatedImageBody} />
    )
}
