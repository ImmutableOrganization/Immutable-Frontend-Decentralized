...
const Chat: NextPage<ChatProps> = ({ socketId, setUserWantsSocketOn, disconnectSocket, userCounter, channelsRef, socketConnected, selectChannel, setEnableChannelWhiteList, enableChannelWhiteList, channelInfo, sendChatMessage, joinRoom, leaveRoom, selectedChannel }) => {
    ...
    return (
        <>
            ...
            <div className={'chat ' + (openExplanationModal ? 'blurred' : '')}>
                <>
                    <ChatInfo socketId={socketId} setUserWantsSocketOn={setUserWantsSocketOn} disconnectSocket={disconnectSocket} socketConnected={socketConnected} userCounter={userCounter} />
                    <ChatExplanationModal />
                    <Frame className={'chat-info'} headerText={"Twitter"} body={() => {
                        return communityBody();
                    }} />
                    <Frame className="chat-info" headerText={"Discord"} body={() => { return discordBody() }} />

                    {channelsRef.current.length > 0 && socketConnected &&
                        <>
                            <div className="chat-footer">
                                <ChatOptions dateHidden={dateHidden} setDateHidden={setDateHidden} timeHidden={timeHidden} setTimeHidden={setTimeHidden} shortenSocketId={shortenSocketId} setShortenSocketId={setShortenSocketId} enableChannelWhiteListRef={enableChannelWhiteList} setEnableChannelWhiteListState={setEnableChannelWhiteList} separateChatAndChannels={separateChatAndChannels} setSeparateChatAndChannels={setSeparateChatAndChannels} openExplanationModal={openExplanationModal} setOpenExplanationModal={setOpenExplanationModal} />
                                <JoinChannel addChannel={addChannel} channelName={channelName} setChannelName={setChannelName} handleAddRoomKeyPress={handleAddRoomKeyPress} />
                            </div>
                            <div className="frame messages">
                                ...
                                <div className="terminal" style={{ 'flexDirection': (separateChatAndChannels ? 'column' : 'row') }}>
                                    <div className={"frame chat-channels " + (!smallScreen && !separateChatAndChannels && 'onechannel')} style={{ 'width': '100%', 'height': (separateChatAndChannels ? '25vh' : '50vh') }}>
                                        {channelsRef.current.map((_channel: Channel, index: number) =>

                                            <div key={index} className={"button with-x " + `${selectedChannel !== undefined && channelsRef.current[selectedChannel] && _channel.channel_name === channelsRef.current[selectedChannel].channel_name && 'selected'}`} onClick={() => selectChannel(_channel.channel_name)} >
                                                <div className='channel-title' key={_channel.channel_name}  >{_channel.channel_name}</div>
                                                <span>
                                                    {channelInfo[_channel.channel_name] && channelInfo[_channel.channel_name].has_unread_messages && index !== selectedChannel &&
                                                        <button type='button' className="button"> <img src="https://gateway.pinata.cloud/ipfs/QmR3abnQmUWuS3tUWSdKMs7uSnjVtGTGSjxgYK3GgLgEgf/triangle-exclamation-solid.svg" className='icon' /> </button>
                                                    }
                                                    <button type='button' className="button leaveChannel" onClick={(event) => leaveRoom(event, _channel.channel_name)}>
                                                        <img src="https://gateway.pinata.cloud/ipfs/QmR3abnQmUWuS3tUWSdKMs7uSnjVtGTGSjxgYK3GgLgEgf/xmark-solid.svg" className='icon' />
                                                    </button>
                                                </span>
                                            </div>
                                        )}
                                    ...
                                    <div className="chat-pane">
                                        {selectedChannel === undefined ?
                                            (
                                                <div>
                                                    You must select a channel to chat.
                                                </div>
                                            )
                                            :
                                            (
                                                <>
                                                            ...
                                                            {index === selectedChannel &&
                                                                <div key={index}>
                                                                    ...
                                                                    <div className="socketMessages">
                                                                        {_channel.messages.slice(0)
                                                                            .map((_messages: Message, _msgIndex: number) =>
                                                                                <div key={`${index}:${_msgIndex}`}>
                                                                                    {
                                                                                        _messages.sender !== "Server" ?
                                                                                            <u onClick={() => selectChannel(_messages.sender)}>{!shortenSocketId ? _messages.sender : _messages.sender.substring(0, 8)}</u>
                                                                                            :
                                                    ...
                                                    <div className="send-message-form">
                                                        <input className='text_input terminal-input' type='text' onKeyDown={(e) => handleKeypress(e)}
